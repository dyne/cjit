...
path=${1:?usage: embed-asset-path.sh PATH [EMBED_NAME]}
parent=$(dirname "$path")
...
if(CJIT->fresh) res = muntarfs_extract_targz_to_path(CJIT->tmpdir,(const uint8_t*)&${varname},${varname}_len);
