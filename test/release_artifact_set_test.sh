...
for os in macos-14 macos-15 macos-26; do mkdir -p "$temporary/release-osx-$os-arm64"; printf x > "$temporary/release-osx-$os-arm64/cjit-Darwin-$os-arm64"; done
...
truncate -s 0 "$temporary/release-osx-macos-15-arm64/cjit-Darwin-macos-15-arm64"
if ./test/release_artifact_set.sh "$temporary"; then printf 'Accepted empty macOS artifact\n' >&2; exit 1; fi
printf x > "$temporary/release-osx-macos-15-arm64/cjit-Darwin-macos-15-arm64"
mkdir -p "$temporary/release-osx-macos-15-x86_64"
printf x > "$temporary/release-osx-macos-15-x86_64/cjit-Darwin-macos-15-x86_64"
