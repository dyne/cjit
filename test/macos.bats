...
@test "macOS builds and links a dylib from whitespace paths" {
    ...
    run ${CJIT} -q -L"${work}" -lanswer -o "${executable}" "${work}/main.c"
    assert_success
    [ -x "${executable}" ]
    run file "${executable}"
    assert_success
...
@test "macOS linker failure leaves no output file" {
    case "$(uname -s)" in Darwin) ;; *) skip "macOS native contract only applies on Darwin" ;; esac
    executable="${TMP}/missing native/program"
    run ${CJIT} -q -o "${executable}" test/hello.c -lmissing_cjit_native_library
    assert_failure
    [ ! -e "${executable}" ]
}
